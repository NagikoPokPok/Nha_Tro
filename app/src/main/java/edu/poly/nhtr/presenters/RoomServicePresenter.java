                ...
                .addOnCompleteListener(task -> {
                    if(task.isSuccessful() && task.getResult()!=null && !task.getResult().isEmpty()){
                        for(QueryDocumentSnapshot document : task.getResult()){
                            ...
                            roomService.setService(getService(roomService, roomService1 -> {
                                final int[] quantity = {0};
                                try {
                                    quantity[0] = Math.toIntExact(document.getLong(Constants.KEY_ROOM_SERVICE_QUANTITY));
                                } catch (Exception e) {
                                    if(roomService1.getService() != null){
                                        if(roomService1.getService().getFee_base() == 1){
                                            quantity[0] = 1;
                                        } else if (roomService1.getService().getFee_base() == 2) {
                                            getNumberOfGuest(roomId, quantityOfGuest -> quantity[0] = quantityOfGuest);
                                        } else if (roomService1.getService().getFee_base() == 0) {
                                            quantity[0] = -1;
                                        }
                                    }
                                }
                                roomService1.setQuantity(quantity[0]);
...
//    }
    ...
    public void getAvailableService(String homeId, List<RoomService> roomServices, OnGetAvailableServiceListener callback) {
...
//            Log.e("serviceList", roomService.getService().getName());
                            ...
                            if (serviceName != null && (serviceName.equalsIgnoreCase("điện") || serviceName.equalsIgnoreCase("nước"))) {
                                ...
                                FirebaseFirestore.getInstance().collection(Constants.KEY_COLLECTION_ROOM_SERVICES_INFORMATION)
                                        ...
                                        .addOnCompleteListener(task1 -> {
                                            if (task1.isSuccessful()) callback.onRoomServiceAutoSet();
                                        });
                            }
                        }
                    }
    ...
    public void updateDataBeforeAdd(ServiceAdapter adapter, List<RoomService> roomServices, List<Service> services) {
        ...
        for (Service service : listServiceAdd){
            setNewRoomServiceQuantity(roomId, service, quantity -> {
                HashMap<String, Object> data = new HashMap<>();
                data.put(Constants.KEY_ROOM_ID, roomId);
                ...
                if (listServiceAdd.size() == count[0]){
                    batch.commit().addOnCompleteListener(task -> {
                        if (task.isSuccessful()){
                            getRoomServices(roomId, listRoomService -> {
                                roomServices.clear();
                                roomServices.addAll(listRoomService);

                                // Remove listServiceAdd in services
                                for (Service service1 : listServiceAdd){
    ...
    private void setNewRoomServiceQuantity(String roomId, Service service, OnSetNewRoomServiceQuantityListener callback) {
        if(service.getFee_base() == 1){
            ...
            getNumberOfGuest(roomId, callback::onSetNewRoomServiceQuantity);
